...
function output(verdict: Verdict, extra: Record<string, unknown> = {}) {
  console.log(
    JSON.stringify(
      {
        verdict,
        browserAudioCaptured: false,
        gatewayAudioReceived: false,
        telegramAudioInjected: false,
        calleeConfirmedAudio: false,
        checks,
        ...extra,
      },
      null,
...
async function main() {
  ...
  record(
    "media-bridge-ready",
    health.mediaBridgeReady === true ? "PASS" : "FAIL",
    `mediaBridgeReady=${String(health.mediaBridgeReady)}`,
  );

  if (checks.some((check) => check.status === "FAIL")) {
    output("BLOCKED", {
      reason:
        "C3 requires a selected native Telegram VoIP media engine and browser WebRTC receiver; current gateway reports mediaBridgeReady=false.",
    });
    return;
  }
  ...
  output("BLOCKED", {
    reason:
      "C3 WebRTC/media bridge is not implemented in this gateway build; do not claim one-way audio PASS.",
  });
}
